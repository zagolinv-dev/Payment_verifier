...
export default function UsersPage() {
  ...
  const [newUser, setNewUser] = useState({ email: "", password: "", fullName: "", role: "WAITRESS" });
  ...
  const handleAddUser = async (e) => {
    ...
    try {
      ...
      setNewUser({ email: "", password: "", fullName: "", role: "WAITRESS" });
      loadUsers();
              ...
              <tbody className={`divide-y font-medium ${darkMode ? "divide-white/[0.04] text-zinc-300" : "divide-black/5 text-zinc-700"}`}>
                {filtered.length === 0 ? (
                  ...
                  <tr key={user.id} className={`transition-colors ${darkMode ? "hover:bg-white/[0.02]" : "hover:bg-zinc-50"}`}>
                    ...
                    <td className={`p-4 sm:p-5 font-mono ${darkMode ? "text-zinc-500" : "text-zinc-400"}`}>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        <select
                          ...
                          className={`text-[10px] px-2 py-1.5 rounded-lg border font-bold outline-none cursor-pointer transition-all ${
                            darkMode
                              ? "bg-[#080E1A] border-white/10 text-white focus:border-emerald-500/50"
                              : "bg-zinc-50 border-black/10 text-zinc-900 focus:border-emerald-500/50"
                          }`}
                        >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowAddModal(false)}>
            <div className={`relative w-full max-w-md rounded-2xl overflow-hidden shadow-2xl border transition-all ${
              ...
              <div className={`px-6 py-4 border-b flex items-center justify-between ${
                ...
                <button onClick={() => setShowAddModal(false)} className={`text-lg hover:opacity-80 font-bold p-1 cursor-pointer ${darkMode ? "text-zinc-400" : "text-zinc-600"}`}>
                  ✕
                </button>
              </div>
              <form onSubmit={handleAddUser} className="p-6 space-y-4">
                <div>
                  ...
                  <input
                    type="text" value={newUser.fullName}
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-zinc-950 font-bold text-xs shadow-lg shadow-emerald-500/20 hover:from-emerald-400 hover:to-emerald-500 transition-all cursor-pointer"
                  >
                    Create User
                  </button>
                </div>
